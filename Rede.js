...
var Rede = /** @class */ (function () {
    ...
    Rede.prototype.criarConta = function () {
        ...
        readlineSync.question("Conta criada com sucesso! ID: ".concat(novaConta.getId()));
    ...
    */
    Rede.prototype.puxarPosts = function (contaLogada) {
        for (var _i = 0, _a = this.contas; _i < _a.length; _i++) {
            var conta = _a[_i];
            if (contaLogada.getSeguindo().includes(conta.getId())) {
                if (conta.getPosts().length > 0) {
                    for (var _b = 0, _c = conta.getPosts(); _b < _c.length; _b++) {
                        var post = _c[_b];
                        console.log("*********************************************");
                        console.log("".concat(conta.getNomeUsuario, ": ").concat(post.getConteudo, "\n                                    Curtidas: ").concat(post.getCurtidas.length));
                    }
                }
            }
        }
